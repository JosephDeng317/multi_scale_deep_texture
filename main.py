...
import os
from lib.model import data_loader, Synthesis
from lib.ops import *
import time
import numpy as np
import scipy.misc
...
parser.add_argument(
    '--texture_shape',
    help='img_size of synthesis output texture, if set to [-1,-1], the shape will be the same as sample texture image',
    nargs='+',
    type=int
)
...
for i in range(pyrm_layers - 1, -1, -1):
    ...
    w, h = w0 // (2**i), h0 // (2**i)
    # Using tf.image.resize with bicubic method
    target = tf.image.resize(targets, [w, h], method=tf.image.ResizeMethod.BICUBIC)
    ...
    if i == pyrm_layers - 1:  # initialize large scale gen_output
    ...
    if i == 0:
        ...
        scipy.misc.toimage(np.squeeze(gen_output), cmin=-1.0, cmax=1.0).save(
            os.path.join(FLAGS.output_dir, '%s_%i_%i_%i_%.1f.png' % (tar_name, pyrm_layers,
                                                                       gen_output.shape[1],
                                                                       gen_output.shape[2],
                                                                       total_time)))
    # tf.reset_default_graph() removed in TF2
